...
export default function Home() {
  ...
  const handleGenerateScript = async () => {
    ...
    let endpoint = 'http://107.175.89.173:8000/api/generate-from-text';
    ...
    if (activeTab === 'text') {
      ...
      endpoint = 'http://107.175.89.173:8000/api/generate-from-video';
    }
  ...
  const handleGenerateAudio = async (textScript: string, voiceName: string) => {
    ...
    try {
      const res = await fetch('http://107.175.89.173:8000/api/generate-tts', {
        method: 'POST',
        body: formData,
      });
